...
- **v0.27.0** — operator-side implementation of SPEC §2.6.4's stop-action matrix. The three "止め方" — *exec を止める* / *job を delete* / *schedule を無効化* — now have first-class API + CLI + SPA hooks instead of requiring multi-command CLI choreography. **(1) Job-delete cascade revoke** ([#36](https://github.com/yukimemi/kanade/pull/36)): `DELETE /api/jobs/{id}` writes `script_status.{id} = REVOKED` before removing the Manifest, so already-published Commands skip via Layer 2 at agent fire time instead of executing against a now-deleted manifest. Revoke-first / delete-second order so a mid-flight failure leaves a safe state; failure post-revoke surfaces `script_status.{id} is already REVOKED — kanade unrevoke {id} to recover` in the error body. **(2) Schedule hard-disable** ([#37](https://github.com/yukimemi/kanade/pull/37)): new `POST /api/schedules/{id}/disable[?cascade=]` and CLI `kanade schedule disable <id> [--cascade]`. Soft disable just flips `enabled = false` (cron stops, in-flight runs); hard disable adds `script_status` revoke of the schedule's `job_id`. Uses `kv.entry().revision` + `update()` for optimistic concurrency — no more clobbering a concurrent cron edit. **(3) SPA stop-actions UI** ([#38](https://github.com/yukimemi/kanade/pull/38)): Revoke / Unrevoke per-row buttons on `/jobs`, soft + hard disable + enable buttons on `/schedules`, Kill button on `/results` (job_id column surfaced from `execution_results.job_id`). New `POST /api/schedules/{id}/enable` matches the disable shape (revision-based update, no full-row re-POST). Per-row pending state tracked via `Set<string>` updated in `onMutate` / `onSettled` so concurrent clicks across rows don't grey each other out. SPA tables also got: theme-color thin scrollbars (v0.25), `max-w-screen-2xl` + tighter padding (v0.26), and now Layer 2 + Layer 3 buttons within reach of any panic-window operator. PRs went through Gemini Code Assist + CodeRabbit review (2 rounds each on #36/#38).

- **v0.28.0** — operator UX polish on the v0.27 stop-actions UI, driven by hands-on feedback. **(1) Result detail page** ([#44](https://github.com/yukimemi/kanade/pull/44)): new `/activity/:requestId` route renders the full `ExecResult` payload (metadata grid, per-stream cards with copy-to-clipboard, `max-h-[60vh]` scroll). Activity row's request_id is a `<Link>`, so Ctrl/⌘+click opens a new tab — operators can lay out multiple results side-by-side via browser tabs / window split to diff stdout/stderr across PCs. Inline "show more" button on the table row falls back to lazy-fetched full body for quick peeks without leaving the list. Shared `fmtIsoLocal` helper lifted into `lib/utils.ts`. **(2) Results → Activity rename** ([#47](https://github.com/yukimemi/kanade/pull/47)): "Results" implied only post-completion outcomes; the page now reads as "what's happening + what happened" so the upcoming Running view (queued for v0.29+) slots in naturally. Back-compat redirects from `/results` and `/results/*` keep existing bookmarks working. **(3) Revoke status visibility**: new `GET /api/scripts/status` returns `{ cmd_id: "ACTIVE" | "REVOKED" }`; the Jobs page renders an active/revoked badge per row, disables revoke when already revoked (and vice versa), and invalidates the status query on each mutation so the badge flips immediately. **(4) Plain-language confirm dialogs**: dropped "Layer 2 check", "SPEC §2.6.2", "script_status.X = REVOKED" jargon from revoke / delete / hard-disable prompts. Operators shouldn't need spec section numbers to click a button. **(5) Activity hides Kill button on finished rows**: result rows always carry `finished_at`, so the kill control was a dead button — now hidden unless `r.job_id && !r.finished_at`, lighting up automatically once the Running view lands. Bot review trail through Gemini Code Assist + CodeRabbit covered both PRs.

- **v0.29.0** — foundation rework that fixes three patchwork data-model bugs that had been silently degrading multi-PC deployments (closes [#19](https://github.com/yukimemi/kanade/issues/19) + α — one coordinated wire migration instead of three half-fixes). **(1) Per-PC fan-out results stop being dropped.** `execution_results` PK migrated from `request_id` to a new agent-minted `result_id` (per (Command, PC) UUID). Pre-v0.29: a `commands.all` / `commands.group.X` publish shares one `request_id` across every responding agent, and the projector's `ON CONFLICT(request_id) DO NOTHING` silently discarded every result past the first PC. After the migration both rows persist — broadcast Activity views finally show every PC that replied. Migration backfills legacy rows with `result_id = request_id`, so old `/activity/:id` deep links keep resolving. **(2) `Command.job_id` → `Command.exec_id` rename.** The field always carried a per-deploy UUID (= `executions.exec_id`), never the job-catalog id — the name was actively misleading anyone reading wire dumps. `serde(alias = "job_id")` on the new field keeps STREAM_EXEC replays from pre-v0.29 backends decodable across the upgrade window; the `kanade run --job-id` CLI flag stays usable via clap alias. Subject helper `subject::kill(exec_id)` doc-clarified — the name on the wire didn't change. **(3) `ExecResult.exec_id` wires up the unused `executions` counters.** Pre-v0.29 the agent never forwarded `Command.exec_id` (then `job_id`) into the result, so the projector had no way to attribute a result back to its deployment row. `executions.success_count` / `failure_count` had sat at 0 since v0.16 as a result. The agent now copies `exec_id` from Command → ExecResult, and the projector's UPDATE bumps the counters + recomputes `status` (`pending` → `running` → `completed` when `success+failure >= target_count`). New `GET /api/executions` + `/api/executions/{exec_id}` surface the live counters for the upcoming Running view. **(4) `/api/jobs/{job_id}/kill` actually kills now.** Pre-v0.29 the route published `kill.{cmd_id}` which no agent subscribes to (agents listen on `kill.{exec_id}`) — the SPA's kill button had been a silent no-op since v0.27. The route now SELECTs every running `exec_id` for the cmd and publishes `kill.{exec_id}` per deployment. **(5) Migration 0003** does the SQLite table-swap (no in-place PK ALTER), adds an `exec_id` column with index, and recreates v0.19's `job_id` index on the new table. Rust unit tests cover the broadcast-result PK invariant + redelivery dedupe + counter promotion behaviour.

- **v0.30.0** — kill-UX completion + per-PC in-flight visibility. Five PRs ([#71](https://github.com/yukimemi/kanade/pull/71) Jobs page live counters & kill, [#73](https://github.com/yukimemi/kanade/pull/73) unified `execution_results` lifecycle, [#74](https://github.com/yukimemi/kanade/pull/74) browser-local timestamps in SPA, [#77](https://github.com/yukimemi/kanade/pull/77) stale-pending cleanup + compact Jobs chip + condensed Actions, [#78](https://github.com/yukimemi/kanade/pull/78) regex filters on Activity) deliver "see what's running NOW, kill it from one click — and search through completed runs cleanly". **(1) Per-cmd live counters on the Jobs page.** `GET /api/jobs` now returns `live: { running, pending }` aggregated in one GROUP BY against the v0.29 `executions` table. The Jobs catalog table renders `running: N` / `pending: M` chips per row so the operator's "is this script causing trouble?" question is answered at a glance. **(2) Per-cmd kill on the Jobs page.** New Skull-icon Kill button, disabled when `running + pending = 0` so no-op clicks are impossible. Confirm dialog spells out the kill ≠ revoke distinction ("this does NOT block the next schedule tick"). Backed by the v0.29 fan-out route. **(3) Unified `execution_results` lifecycle.** Migration 0004 makes `finished_at` + `exit_code` nullable and adds a `version` column. New `events.started.<exec_id>.<pc_id>` lifecycle event from the agent (mints `result_id` ONCE at `handle_command` entry and threads it through both EventStarted + ExecResult so the backend UPSERTs both into one row). The Activity page gains a "running (in flight)" status filter, an explicit `running` violet badge for the exit-code column when null, and "running…" for finished_at. No tab split — same screen, one filter narrows. **(4) Race-resilient projection.** `INSERT ... ON CONFLICT(result_id) DO UPDATE WHERE finished_at IS NULL` handles arrival in either order: events.started-then-result is the normal path (insert then UPDATE flips finished_at), result-then-events.started (JetStream redelivery race) yields a finished row first that the late start's ON CONFLICT DO NOTHING leaves untouched. Counter bump gated on `rows_affected > 0` so redeliveries don't double-count. **(5) Outbox reliability fixes.** Both agent outboxes (ExecResult + the new EventStarted) gain mkdir retry on the drain loop (previously a startup failure killed the drain forever) and continue-on-file-error in `drain_once` (previously one bad file pinned the whole outbox behind it). **(6) Stale-pending cleanup ([#77](https://github.com/yukimemi/kanade/pull/77)).** Backend task every 5 min flips `executions.status = 'pending'` rows older than 1 h to `'expired'` so the Jobs live chip stops accumulating fires whose `ExecResult` never lands (offline targets, `run_as: user` with no session, deadline-missed). The chip query is unchanged (`status IN ('pending', 'running')`); expired rows drop out naturally. **(7) Jobs page UX polish ([#77](https://github.com/yukimemi/kanade/pull/77)).** `running: N` / `pending: M` text badges → icon + number chips (`▶ N` / `⏳ M`) that fit a narrow column without wrapping. Actions column buttons (kill / revoke / unrevoke / delete) now conditionally rendered — only the buttons applicable to the row's current state appear, reducing visual clutter from 4-stacked to 2-3 visible. **(8) Browser-local timestamps + regex Activity filter** ([#74](https://github.com/yukimemi/kanade/pull/74), [#78](https://github.com/yukimemi/kanade/pull/78)). SPA timestamps render in the operator's local timezone instead of UTC (subtle but stops everyone mentally converting); Activity page gains regex filter inputs on `pc_id` / `manifest_id` for surgical drill-down across large histories. Bot review trail through Gemini Code Assist + CodeRabbit covered all PRs (7 medium + 2 high fixes plus a self-spotted ghost-row race in the original two-table design that drove the redesign into a unified table).

- **v0.30.1** — release-blocker hotfix for the v0.30.0 SPA build. The Jobs page pending chip used `<Badge variant="secondary">`, but `Badge`'s variant union doesn't include `secondary` (`Button` does — the typo was carried unnoticed since the v0.30.0 Jobs chip was first added in #71). Per-PR CI runs only `cargo` and never built the SPA, so the error went undetected until `release.yml`'s tag-driven cross-compile path tried `tsc -b && vite build` on the v0.30.0 tag and aborted at `error TS2322: Type '"secondary"' is not assignable`. Fix: `variant="secondary"` → `variant="amber"` (semantically fits "pending = waiting in queue" anyway; one-line change in `Jobs.tsx`). The v0.30.0 tag remains pointed at the broken commit but no GitHub Release was ever published from it — v0.30.1 ships the same kill-UX features against a working SPA build. Follow-up tracked: surface SPA build into per-PR CI so this class of regression fails fast next time, not at release time.

- **v0.32.0** — inventory-sw (registry + AppX) + per-PR SPA build CI + SPA in-browser YAML editor + Client App / KLP spec. **(1) `inventory-sw.yaml` manifest** ([#99](https://github.com/yukimemi/kanade/pull/99)). New sample inventory probe combining three software sources into one `apps[]` array — `x64` (HKLM `Uninstall` hive), `wow6432` (`WOW6432Node`), and `appx` (`Get-AppxPackage -AllUsers` for UWP / Store apps, framework + resource packages filtered out). Without AppX, Store-installed Edge / vendor UWP utilities / system apps were invisible to inventory. Identity model uses a per-source stable `key` field (registry's `PSChildName`, AppX's `PackageFamilyName`) rather than `DisplayName` for `explode.primary_key` — version updates and language-pack renames no longer emit spurious `removed + added` history pairs (Gemini #99 review). Same `display: type: table` + `explode:` + `track_history: true` shape as `inventory-hw.yaml`. Defensive `Where-Object { $_ -ne $null }` filter (CodeRabbit #99 review) protects against synthetic null rows when one source returns nothing. **(2) Per-PR SPA build CI** ([#99](https://github.com/yukimemi/kanade/pull/99), closes [#94](https://github.com/yukimemi/kanade/issues/94)). New `spa-build` job in `.github/workflows/ci.yml` runs `bun install --frozen-lockfile && bun run build` against `crates/kanade-backend/web` on every PR, matching what `release.yml` cross-compiles on tag push. Catches `tsc -b && vite build` regressions at PR review time (= what v0.30.0 / v0.31.0 release blockers needed). Job has explicit `permissions: contents: read` for least-privilege scope. **(3) In-browser YAML editor** ([#98](https://github.com/yukimemi/kanade/pull/98)). SPA Jobs / Schedules pages gain an inline editor backed by the `JsonSchema` derive set added in #82 — manifests can be edited without round-tripping through `kanade exec` / a local YAML file. Schema served at `/api/schemas/manifest.json` drives the editor's auto-complete + validation. **(4) End-user Client App + KLP spec** ([#100](https://github.com/yukimemi/kanade/pull/100)). `docs/SPEC.md` gains a new section laying out an end-user-facing Client App (Windows tray companion) and the Kanade Local Protocol (KLP) it speaks to the agent — a documented surface for self-service operations the end user is allowed to trigger without going through Admin. Spec only at this stage; no implementation. **(5) Release workflow refactor** ([#101](https://github.com/yukimemi/kanade/pull/101)). `release.yml` swapped from the hand-rolled per-binary cross-compile matrix to the `pj-rust-workspace` template from `pj-presets` — same outputs (9 binary assets, crates.io publish, GitHub Release), much less per-repo workflow code to maintain. Caught up to where `kata`, `renri`, `todoke` already are.

- **v0.31.1** — release-blocker hotfix for v0.31.0. Two manifest types added by the v0.31 inventory chain (`ExplodeSpec` and `ExplodeColumn` in `crates/kanade-shared/src/manifest.rs`, introduced in [#85](https://github.com/yukimemi/kanade/pull/85) for cross-PC search) didn't carry `#[derive(schemars::JsonSchema)]`. The SPA-editor schema endpoint added in [#82](https://github.com/yukimemi/kanade/pull/82) puts a `JsonSchema` derive on `InventoryHint`, which recurses into `explode: Option<Vec<ExplodeSpec>>` — the trait bound failed to satisfy and the release build aborted with `the trait bound 'ExplodeSpec: JsonSchema' is not satisfied`. The reason each PR's per-PR CI was green: #82 was cut against main BEFORE the explode types existed, the explode PRs were cut against main BEFORE #82 merged, and GitHub doesn't re-run a PR's CI when the base branch advances. `release.yml` on the v0.31.0 tag push was the first build of the combined post-merge state. Fix: add `schemars::JsonSchema` to both derive lists (matches every other Manifest sub-struct, no behaviour or wire-format change). The v0.31.0 tag stays pointed at the broken commit but no GitHub Release was published from it — v0.31.1 ships the same inventory features against a working build. Same forward-fix pattern as v0.30.0 → v0.30.1. Follow-up [#94](https://github.com/yukimemi/kanade/issues/94) (surface SPA build into per-PR CI) is still pending — would also have caught this class of merge-skew regression. Fix landed in [#95](https://github.com/yukimemi/kanade/pull/95).

- **v0.31.0** — inventory: SPA renderer + cross-PC search + change history. Four issues closed together ([#43](https://github.com/yukimemi/kanade/issues/43), [#39](https://github.com/yukimemi/kanade/issues/39), [#40](https://github.com/yukimemi/kanade/issues/40), [#41](https://github.com/yukimemi/kanade/issues/41)) deliver the data plumbing operators need to USE inventory facts beyond the per-PC detail view. **(1) ja-JP agent stdout fix** ([#83](https://github.com/yukimemi/kanade/pull/83)). Agent was silently dropping PowerShell stdout containing CP932 bytes — symptom: `exit_code=0`, empty capture, no `inventory_facts` row, no logs. Three coupled fixes: tolerant `read_to_end` + `from_utf8_lossy` (no more strict-UTF-8 wipe), partial-buffer preservation across read errors with a marker in stderr, PowerShell UTF-8 prelude that forces `[Console]::OutputEncoding` to UTF-8 in both `run_as: system` and `run_as: user`. Retroactively explains why `inventory-sw` worked locally but produced nothing through the agent on ja-JP / DE / KR / CN Windows. **(2) Nested sub-table renderer** ([#84](https://github.com/yukimemi/kanade/pull/84)). `DisplayField` gains recursive `type: table` + `columns` schema; SPA Inventory PC-detail page renders `disks[]` / `apps[]` as an inline sub-table using existing column hints (`bytes` / `number` / `timestamp`) instead of `JSON.stringify`-ing the array. Fleet summary cells collapse to `"N rows"` for compactness. **(3) Cross-PC search via derived flat tables** ([#85](https://github.com/yukimemi/kanade/pull/85)). Inventory manifests can declare `explode:` specs that flatten payload arrays into real SQLite tables (PK = `(pc_id, job_id) + spec.primary_key`). Projector creates the table at registration + first result (CodeRabbit fix: on read too), atomically replaces per-PC rows on every scan. New `GET /api/inventory/{manifest_id}/search/{field}` endpoint with Django-style filter syntax (`name=Chrome`, `version__lt=120`, `__contains` / `__prefix` / `__suffix` with `%`/`_` escape + `ESCAPE` clause, `limit` / `offset` pagination). Identifier safety: operator-supplied identifiers validated against `[A-Za-z_][A-Za-z0-9_]{0,63}` and double-quoted in DDL/DML so reserved words (`order`, `group`) work as column names. Type-driven row decode keyed on `kind: text|integer|real` (unknown values fail fast — no silent TEXT coercion). `ensure_table` cached in `Mutex<HashSet>` so the hot path is one in-memory lookup after first delivery per spec. **(4) Change-only history projection** ([#86](https://github.com/yukimemi/kanade/pull/86)). New `inventory_history` table + `track_history: true` on explode specs. Projector diffs each PC's incoming payload against prior derived-table rows BEFORE the DELETE-then-INSERT replace, writes `added` / `removed` / `changed` events with full before/after JSON. No-op scans produce zero rows; volume tracks fleet churn, not scan cadence. CHECK constraint at the DB layer enforces the `change_kind` enum. Duplicate identities within one payload collapse to one event (history can't diverge from the snapshot). Batch INSERT via `QueryBuilder::push_values`. 90-day retention sweeper added to `cleanup` module. `GET /api/inventory/{manifest_id}/history/pc/{pc_id}` foundation endpoint with `field` / `since` / `limit` params. `inventory-hw.yaml` updated end-to-end: `disks[]` is now declared as `display: type: table` (renderer), `explode:` (search), AND `track_history: true` (change log). Bot review trail through Gemini Code Assist + CodeRabbit covered all four PRs (12 medium + 5 high fixes across the chain).

Backlog: Prometheus metrics, 3000-agent simulation, NATS cluster + replicated backend, Postgres migration, sidebar nav reorganization (Execute / Observe / Manage grouping), surface SPA build into per-PR CI, inventory follow-ups ([#87](https://github.com/yukimemi/kanade/issues/87) Software page, [#88](https://github.com/yukimemi/kanade/issues/88) explode-spec cache, fleet-wide history search, `first_seen` rollout curve, SPA per-PC History tab, scalar field history).

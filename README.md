...
Backlog: v0.13.0 — generalise inventory as scheduled `jobs/inventory-*.yaml` (operator-defined PowerShell probes + display-config-driven UI table). Then Prometheus metrics, 3000-agent simulation, NATS cluster + replicated backend, Postgres migration.

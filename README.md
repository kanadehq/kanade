...
- **v0.31.1** — release-blocker hotfix for v0.31.0. The release.yml SPA build's preceding `cargo build --release` aborted on `the trait bound 'ExplodeSpec: JsonSchema' is not satisfied` when assembling kanade-shared. The chain: [#82](https://github.com/yukimemi/kanade/pull/82) (parallel YAML store + JSON Schema endpoints) added `schemars::JsonSchema` derives to existing manifest types AND merged 11 minutes BEFORE the v0.31.0 release commit. The v0.31 release PR's CI ran against pre-#82 main and was green; GitHub doesn't auto-rerun PR CI when the base branch advances, so the missing JsonSchema derives on the inventory-PR-introduced `ExplodeSpec` / `ExplodeColumn` types never tripped at PR review time. Fix: two one-line additions of `schemars::JsonSchema` to the two structs' derive list. Same operator-visible pattern as v0.30.0 → v0.30.1 (tag created, no GitHub Release attached). v0.31.1 ships the same inventory feature set with the missing derives in place. Tracked as a follow-up: extend [#94](https://github.com/yukimemi/kanade/issues/94) (SPA build in PR CI) to also include a "rebase-then-rerun" trigger on release PRs so this class of base-branch-drift regression catches at merge time, not release time.

...
export function Inventory() {
  ...
  // Keep ?pc=… synced with the URL for sharable links.
  useEffect(() => {
    if (pcId) {
  ...
  const jobsByManifest = useMemo(() => {
    const map = new Map<string, InventoryJob>();
    for (const j of jobsQ.data ?? []) map.set(j.manifest_id, j);
    return map;
  }, [jobsQ.data]);
      ...
      <Card>
        <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-4">
          <div className="space-y-1">
            <Label htmlFor="inv-pc">pc_id</Label>
          ...
          <div className="space-y-1 text-xs text-muted self-end pb-2">
            ...
            <code>inventory:</code> section in their YAML manifest. Add new
            ones with <code>kanade schedule create configs/jobs/&lt;your-probe&gt;.yaml</code>.
          </div>
        </CardContent>
      </Card>
      ...
      {(jobsQ.data ?? []).length === 0 ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Ship a probe with <code>kanade schedule create configs/jobs/inventory-hw.yaml</code>
        ...
        <Card>
          <CardContent className="p-6 text-muted text-sm">
            Pick a pc_id above to see its inventory facts.
          </CardContent>
        </Card>
      ) : factsQ.isLoading ? (
        ...
        <Card>
          <CardHeader>
            <CardTitle>No facts yet for {pcId}</CardTitle>
            <CardDescription>
              The agent hasn't run any inventory-tagged job successfully yet.
              Trigger one with <code>kanade run {pcId} -- &lt;script&gt;</code>{' '}
        ...
        (factsQ.data ?? []).map((fact) => {
          const display = fact.display.length > 0
            ? fact.display
            : jobsByManifest.get(fact.job_id)?.display ?? [];

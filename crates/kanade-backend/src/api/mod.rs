...
mod spa_route_tests {
    ...
    /// Files reaching a restrictable page whose `apiFetch*` URL is built from a
    /// helper instead of a literal, so this guard cannot resolve it and cannot
    /// check it. Listed so a new one is a deliberate act rather than a hole.
    const DYNAMIC_CALL_FILES: &[&str] = &[
        // Query params (os / signing key) chosen at render time.
        "AgentInstall.tsx",
        // Store tab picks the endpoint.
        "Apps.tsx",
        // `bundleUrl(key)` builds the download/delete URL.
        "Collect.tsx",
        // `/api/{groups,pcs}/{name}/config` scope base + `${base}/inherited`.
        "Config.tsx",
        ...
        "Search.tsx",
        // `endpointBase(kind)` + `/{id}/yaml`, for four manifest kinds.
    ...
    fn a_restricted_page_can_fetch_what_it_fetches() {
            ...
            for file in files {
                ...
                for fetch in fetches(&src) {
                    let Some(url) = fetch
                        ...
                        dynamic.insert(file_name.clone());
                        continue;
                    };
        ...
        let dynamic: Vec<&str> = dynamic.iter().map(String::as_str).collect();
        assert_eq!(
            dynamic, DYNAMIC_CALL_FILES,
            "the set of pages whose fetch URL is computed changed — make it a literal, or list \
             it in DYNAMIC_CALL_FILES so the hole is deliberate"
        );
    }
}

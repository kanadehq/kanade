...
pub struct KeyPolicy {
    ...
    pub label: String,
    /// Reject a signature whose message is older than this, when the message
    /// carries a timestamp the caller checks. `None` = no freshness bound
    /// beyond whatever the command itself expresses (`deadline_at`).
    ///
    /// A break-glass key is expected to set a short bound: its signatures
    /// should not stay valid long enough to be captured and replayed later.
...
impl SigHeaders {
    ...
    pub fn is_absent(&self) -> bool {
        self.sig_b64.is_none() && self.kid.is_none() && self.alg.is_none()
    }
}
...
pub fn verify<'a>(
    ...
    // verification fails, never make an invalid signature pass. Keeping it
    // outside is what lets the signature stay over the untouched body.
...
pub fn sign(key: &SigningKey, kid: &str, body: &[u8]) -> SigHeaders {
...
mod tests {
    ...
    fn round_trip_accepts_the_bytes_that_were_signed() {
        ...
        let headers = sign(&sk, "backend-1", body);
        let ok = verify(&ring, body, &headers).expect("verifies");
    ...
    fn a_single_flipped_byte_fails() {
        let (sk, vk) = keypair(1);
        let ring = ring_with("backend-1", vk);
        let headers = sign(&sk, "backend-1", b"run this");
        // The whole point: bytes the backend did not author must not execute.
        assert_eq!(
            verify(&ring, b"run thit", &headers),
    ...
    fn a_forged_command_from_another_key_fails() {
        ...
        let headers = sign(&attacker_sk, "backend-1", body);
        assert_eq!(
            verify(&ring, body, &headers),
            Err(VerifyError::BadSignature {
                kid: "backend-1".into()
            })
        );
    ...
    fn unsigned_is_distinct_from_every_failure() {
        ...
        // migration window.
        assert_eq!(
            verify(&ring, b"anything", &SigHeaders::default()),
    ...
    fn an_unknown_kid_is_its_own_error() {
        let (sk, vk) = keypair(1);
        let ring = ring_with("backend-1", vk);
        let headers = sign(&sk, "backend-2", b"body");
    ...
    fn rotation_is_two_kids_on_one_ring() {
        ...
        for (sk, kid) in [(&old_sk, "backend-1"), (&new_sk, "backend-2")] {
            let headers = sign(sk, kid, b"during the window");
            assert_eq!(
                verify(&ring, b"during the window", &headers).unwrap().kid,
                kid
            );
        }
    }
    ...
    fn policies_are_per_key_not_per_ring() {
        ...
        let headers = sign(&bg_sk, "break-glass", b"emergency");
        let ok = verify(&ring, b"emergency", &headers).unwrap();
        assert!(
        ...
        let h = sign(&sk, "backend-1", b"routine");
        assert!(!verify(&r2, b"routine", &h).unwrap().policy.audit_every_use);
    }
    ...
    fn an_unexpected_algorithm_is_refused_not_assumed() {
        let (sk, vk) = keypair(1);
        let ring = ring_with("backend-1", vk);
        let mut headers = sign(&sk, "backend-1", b"body");
    ...
    fn an_absent_alg_header_means_the_original_shape() {
        ...
        let mut headers = sign(&sk, "backend-1", b"body");
        headers.alg = None;
        assert!(verify(&ring, b"body", &headers).is_ok());
    }
}

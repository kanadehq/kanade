...
//!
//! Every member here is published, so the `path` dependency between them
//! carries a `version` as well — crates.io has to be able to resolve the
//! requirement for somebody who is not building from this checkout. That
//! version is a literal, and nothing in Cargo makes it follow
//! `[workspace.package] version` when a release bumps it.
//!
//! Centralising it in `[workspace.dependencies]` was already done, for
//! exactly the reason its comment gives — one place instead of three.
//! That turned out not to be enough on its own: one place is still a
//! place somebody forgets, and this workspace was at 0.45.4 with the pin
//! reading 0.45.0 when this test was written.
//!
//! **Nothing was red, and that is the point.** `version = "0.45.0"` means
//! `^0.45.0`, so a stale pin keeps resolving through every patch release
//! and stops only at the first release that moves the minor. The sibling
//! repo `yaiba` — same kata preset, same shape — hit exactly that on its
//! v0.17.0: `cargo build` failed with "candidate versions found which
//! didn't match", three releases after its pins were last correct.
//!
...
//! member could hardcode `version = "9.9.9"` and this test passed. A
//! test with a false negative is worse than no test, because it is also
//! a claim.
//!
//! `yaiba` runs the same code for the same reason — both repos come from
//! `pj-presets:rust-workspace`, and the shape is the preset's.
use std::fs;

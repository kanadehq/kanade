...
pub(crate) async fn run_agent() -> Result<()> {
    ...
    // v0.30 / PR α: parallel outbox for `EventStarted` lifecycle
    // events (script-spawn time). Same atomic-write + drain pattern
    // as the ExecResult outbox; separate directory so the existing
    // v0.24 files keep working unchanged across the upgrade.

...
# Clear a leftover staging file from a previous failed run. Leave any
# pre-existing `.old` alone — that's the rollback artifact if an earlier
# swap aborted mid-way.
...
# Best-effort: a still-running `kanade` holds `.old` open, so the delete
# can fail. That's fine — the next run's `-ErrorAction SilentlyContinue`
# above clears it, and a stale `.old` breaks nothing.
